...
class Lighttrack_onnx(object):
    def __init__(self, info, even=0):
        super(Lighttrack_onnx, self).__init__()
        ...
        self.ort_session_x = onnxruntime.InferenceSession(os.path.join(os.path.dirname(__file__),'../..','model_track.onnx'))
    ...
    def update(self, x_crops, target_pos, target_sz, window, scale_z, p, debug=False):
        ort_inputs_x = {'zf': self.zf.astype(np.float32), 
                        'search':self.to_numpy(x_crops).astype(np.float32)}
        outputs = self.ort_session_x.run(None,ort_inputs_x)
        cls_score = torch.Tensor(outputs[0])
        bbox_pred = torch.Tensor(outputs[1])
    ...
    def track(self, state, im):
        ...
        debug = True
        if debug:
            target_pos, target_sz, _, cls_score = self.update(x_crop.cuda(), target_pos, target_sz * scale_z,
                                                              window, scale_z, p, debug=debug)
            state['cls_score'] = cls_score
        else:
            target_pos, target_sz, _ = self.update(x_crop.cuda(), target_pos, target_sz * scale_z,
                                                   window, scale_z, p, debug=debug)
